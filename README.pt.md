...
Em agosto de 2026, a campanha eleitoral oficial já começou. Em **4 de outubro**, 158.745.463 eleitoras e eleitores estarão aptos a escolher representantes para seis cargos; um eventual segundo turno para presidente e governadores ocorrerá em **25 de outubro**. As datas e o eleitorado são dados do [Tribunal Superior Eleitoral (TSE)](https://www.tse.jus.br/comunicacao/noticias/2026/Julho/mais-de-158-milhoes-de-eleitores-estao-aptos-votar-nas-eleicoes-2026).

O Brasil publica uma quantidade expressiva de dados sobre candidaturas e atuação parlamentar. O desafio é transformar arquivos, APIs, identificadores e documentos distribuídos em informação que uma pessoa consiga realmente explorar. **IRIS Political Insight** usa InterSystems IRIS, dados oficiais, Hybrid Search e Retrieval-Augmented Generation (RAG) para criar essa camada de acesso em linguagem natural.

 **plataforma de inteligência sobre dados públicos eleitorais e políticos**: integra, estrutura, relaciona, indexa, recupera e contextualiza evidências oficiais, mantendo neutralidade partidária e rastreabilidade até a fonte.

## Eleições 2026 encontram Inteligência Artificial

O eleitorado brasileiro fará escolhas para Presidência da República, governos estaduais, Senado, Câmara dos Deputados e assembleias legislativas — ou Câmara Legislativa, no Distrito Federal. Ao mesmo tempo, informações relevantes estão separadas entre órgãos, formatos e modelos de acesso.
...
> “Quais proposições deste candidato tratam de educação pública?”

> “Quais temas aparecem com maior frequência nas proposições deste candidato?”

> “Qual histórico parlamentar está disponível para este candidato?”

> “Quais candidatos possuem evidências relacionadas à redução da jornada 6x1?”

> “Quais fontes oficiais sustentam a resposta?”

O modo individual rejeita evidências de outra pessoa. O modo de descoberta consulta a base global, diversifica resultados entre candidatos e mantém cada trecho ligado à identidade correspondente. O sistema não transfere proposições entre pessoas nem deduz uma candidatura pelo nome de um autor.

## Por que isso importa agora?

A propaganda eleitoral está autorizada desde **16 de agosto**, conforme o [calendário oficial do TSE](https://www.tse.jus.br/comunicacao/noticias/2026/Marco/eleicoes-2026-confira-as-principais-datas-do-calendario-eleitoral). O projeto está sendo construído durante o processo eleitoral, não para um cenário futuro.

Seu interesse público está em reduzir a distância entre **dados publicados** e **informação explorável**. A plataforma não recomenda candidaturas, não atribui notas ou ideologias, não prevê resultados e não produz rankings políticos. Ela distingue ausência de evidência de evidência de ausência, mostra os trechos e links usados e mantém postura apartidária.

## Como funciona

~~~mermaid
flowchart LR
    A[Dados oficiais<br/>TSE + Câmara] --> B[Ingestão e vínculo<br/>de identidade]
    B --> C[(InterSystems IRIS<br/>relacional + streams + vetores)]
    C --> D[Busca lexical]
    C --> E[Vector Search]
    D --> F[Hybrid Search<br/>RRF]
    E --> F
    F --> G[Evidências enriquecidas<br/>E1...En]
    G --> H[RAG<br/>prompt restritivo + LLM]
    H --> I[Resposta fundamentada<br/>+ fontes oficiais]
~~~
...
IRIS não é apenas onde o projeto guarda dados. Ele reúne no mesmo backend as representações e caminhos de acesso necessários:
...
flowchart TB
    ...
    OBJ --> DB[(IRISAPP)]
    SQL --> DB
    DB --> REL[Dados relacionais]
    DB --> STR[Streams]
    DB --> VEC[%Vector]
    VEC --> RET[VECTOR_COSINE]
    REL --> RET
    RET --> RRF[Lexical + Vector + RRF]
    RRF --> RAG[Contexto + LLM]
    RAG --> UI
~~~

Não existe container <code>api</code> nem Waitress. O IRIS serve a API Flask; o segundo container contém somente a interface Streamlit.
